...
本项目 `lua-5.4.8/` 目录下包含 Lua 5.4.8 的完整官方源码，作为修改时的参考依据。

### 路径速查

| 文件 | 内容 |
|------|------|
| `lua-5.4.8/src/lua.h` | 核心 C API 声明（`lua_*` 函数、类型常量） |
| `lua-5.4.8/src/lauxlib.h` | 辅助库 API 声明（`luaL_*` 函数） |
| `lua-5.4.8/src/lualib.h` | 标准库声明 |
| `lua-5.4.8/src/lobject.h` | 内部对象结构体与宏 |
| `lua-5.4.8/src/lstate.h` | 状态机内部结构 |
| `lua-5.4.8/src/liolib.c` | I/O 库参考实现 |
| `lua-5.4.8/src/ltablib.c` | Table 库参考实现 |
| `lua-5.4.8/src/lvm.c` | 虚拟机执行逻辑 |
| `lua-5.4.8/src/lgc.c` | 垃圾回收实现 |
| `lua-5.4.8/src/ltm.c` | 元表方法查找逻辑 |
| `lua-5.4.8/doc/manual.html` | Lua 5.4 参考手册（HTML 版） |

### 使用规则

修改本项目中的任何文件时，都应结合 Lua 源码验证内容的准确性：

#### 修改 `.c` 示例

- 对照 `lua.h` / `lauxlib.h` 确认函数签名、参数列表、返回值类型正确
- 对照头文件确认宏常量（`LUA_T*`、`LUA_OK` 等）拼写正确
- 涉及内部机制时，在 `lobject.h` / `lstate.h` 中验证结构体布局

#### 修改 `.md` 文档或翻译

- 文档中出现的 API 函数名、参数名、常量名必须与 `lua.h` / `lauxlib.h` 中的声明一致
- 行为语义描述（如 GC 规则、栈操作、metatable 查找）需在对应实现文件（`lvm.c`、`lgc.c`、`ltm.c`）中找到依据
- 翻译时保持 API 术语统一，以头文件中的官方命名和文档中的首次翻译为准

#### 通用验证流程

1. **搜索定位**：`grep -n "目标API" lua-5.4.8/src/lua.h` 确认签名与拼写
2. **交叉检查**：文档/注释描述与源码实现逻辑是否一致
3. **编译验证**：修改 `.c` 后运行 `meson compile -C build`（零警告）
4. **测试验证**：修改 `.c` 后运行 `meson test -C build`（全部通过）
5. **文档构建**：修改 `.md` 后运行 `mdbook build`（无报错）
